...
def calc_exp(strvar):
	...
	# ### part1 只计算乘除
	while True:  #循环匹配乘除和加减  然后计算内括号表达式的值   #关键点1
		...
		if obj:
			...
			# 3.用结果替换原乘除表达式
			strvar = strvar.replace(res1,str(res2))  #int转成str 
			print(strvar , "strvar ... 1111")
	...
	lst = re.findall(r'[+-]?\d+(?:\.\d+)?',strvar)  #['-5', '+6.0']  正则3
	# lst = re.findall(r'[+-]?\d+(\.\d+)?',strvar)  #['', '.0']
	print(lst)  #['-5', '+6.0']	
...
def remove_bracket(strvar):
	while True:  #循环匹配内括号  关键点2
		...
		# obj = re.search(r'\([^\(\)]\)',strvar)  #少了+  不然就是()
		...
		if obj:
			...
			# 计算括号里面的表达式
			res2 = calc_exp(res1)
			print(res2,"res2 ... remove_bracket")
			#1.0 res2 ... remove_bracket
			
			# 用计算的结果替换原来的括号(及其里面的表达式)
			strvar = strvar.replace(res1,str(res2))  #res2从int转成str
			print(strvar) 
			#1-2*1.0+(40-7)	  #第一次循环
			# 1-2*1.0+33.0	  #第2次循环		
		else: #注意缩进
