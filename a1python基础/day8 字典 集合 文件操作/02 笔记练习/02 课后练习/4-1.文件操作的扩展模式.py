# ### 文件操作的扩展模式
# utf-8编码格式下,默认一个中文三个字节,一个英文或符合,占用一个字节
# read()  功能:读取字符的个数(里面的参数代表字符个数)   字符
	# 注意:从当前光标往右边读
# seek()   功能:调整指针的位置(里面的参数代表字节个数)  字节
	# seek(0)	  把光标移动到文件的开头
	# seek(0,2) 把光标移动到文件的末尾
# tell()	 功能:当前光标左侧所有的字节数(返回字节数)  字节
...
print('你'.encode())  #b'\xe4\xbd\xa0'


# 8. with 语法 自动实现文件关闭操作
# 方法一  读取二进制字节流
