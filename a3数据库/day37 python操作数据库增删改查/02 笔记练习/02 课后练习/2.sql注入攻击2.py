...
# (1) sql注入的现象
# ''' 现象:绕开账号密码登录成功 '''
usr = input('请输入您的用户名>>>')  #输入  2222' or 4=4 -- aaa #aaa前面需要一个空格
pwd = input('请输入您的密码>>>')   #输入空
...
sql2 = "select * from usr_pwd where username= '%s' and password='%s'" % (usr,pwd)
# 有sql注入的风险
...
# 相当于 : select * from usr_pwd where 10=10; 绕开了账户和密码的判断 -- 代表的是注释 后面的内容被注释了;

# res = cursor.execute(sql1)  #建表
# res = cursor.execute(sql11)  #插入一条数据  #必须在表里插入至少一条数据.否则无法模拟sql注入
...
cursor = conn.cursor()
sql = 'select * from usr_pwd username=%s and password=%s'
