...
# 方法1 
select
	t2.sid,t2.sname,avg(num) as avg_num
from
...
select
	t3.sname,t1.num
from
	score as t1, course as t2,student as t3
where
	t1.student_id = t3.sid and
	t1.course_id = t2.cid and
	t2.cname = '生物' and
	t1.num < 60;
...
# 2.学习李平老师课程的学生中,找平均成绩最高的学生id   in
select
...
# 连一张学生表,顺手查姓名
select
	student_id,sname,avg(num) as avg_num
from
	score as t1 , student as t2
where
...
# 3.综合拼接
select
	t1.student_id,num,t1.course_id
from
	score as t1,(select
		course_id,max(num) as max_num
	from
...
# 4 连学生姓名
select
	t1.student_id,t3.sname,num,t1.course_id
from
	score as t1,(select
		course_id,max(num) as max_num
	from
		score
	group by
		course_id) as t2,student as t3
where
	t1.course_id = t2.course_id and
	t1.student_id = t3.sid and
	t1.num = t2.max_num;
	
 # student_id | sname  | num | course_id |
# +------------+--------+-----+-----------+
# |          3 | 张三   |  87 |         3 |
# |          4 | 张一   | 100 |         4 |
# |          5 | 张二   | 100 |         4 |
...
# 2 连表
select
	t1.course_id,num,group_concat(t2.sname)
from
	score as t1,student as t2,(select
		course_id,max(num) as max_num
	from
		score
	group by
...
where
	t1.student_id = t2.sid and
	t1.course_id = t3.course_id and
	t1.num = t3.max_num
group by
	t1.course_id;
	
 # course_id | num | group_concat(t2.sname) |
# +-----------+-----+------------------------+
# |         1 |  91 | 李一                   |
# |         2 | 100 | 铁锤,李三,张四         |
# |         3 |  87 | 张三,刘三              |
...
select
	...
	t2.num	
from
	score as t1,score as t2
where
	t1.num = t2.num and
	t1.course_id > t2.course_id;
	
# student_id | student_id | course_id | course_id | num | num |
# +------------+------------+-----------+-----------+-----+-----+
# |          1 |          6 |         2 |         1 |   9 |   9 |
# |          1 |          7 |         2 |         1 |   9 |   9 |
# |          1 |          8 |         2 |         1 |   9 |   9 |
...
# 2.学过 1 3 4 学科的学生都有哪些id   in
select
	distinct(student_id),t2.sname
from
	score as t1,student as t2
where
	t1.student_id = t2.sid and
