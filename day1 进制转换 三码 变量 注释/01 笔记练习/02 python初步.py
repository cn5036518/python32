...
2.6.2 不同文件(模块)里,部分数据驻留小数据池中(仅对python3.6版本负责)
小数据池只针对:
    int str bool 空元组() None关键字 这些数据类型有效
...
#(2)对于str而言
    ...
    3.用*号得到的字符串,分两种情况.
      ...
      2)乘数大于1时:乘数大于1,仅包含数字,字母,下划线时会被缓存,
                    但字符串长度不能大于20
                    
#(3) 指定驻留
    # 从 sys模块 引入 intern 函数  让a,b 两个变量指向同一个值
