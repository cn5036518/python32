...
B/S 架构 : 
	b => Brower  (浏览器)
	谷歌,火狐,360,ie
	通过输入网址,访问对方的服务器,对方服务器响应之后,
	把数据返回,浏览器通过返回的数据,渲染页面,看到最后的结果
	s => server (服务端)
	一旦开启,永不关机(除非宕机)
...
# 网段的作用:
			...
			# 在一个网段里可以不通过因特网,直接对话
# 判别的依据：
			# 如果IP地址和子网掩码相与(and)得到的值相同就是同一网段
# """
# """
# 逻辑与
# 全真则真,一假则假
...
数据链路层:
	封装mac地址:
		...
		arp协议:不知道mac 通过ip去找mac
		rarp协议:不知道ip 通过mac去找ip
...
# """arp协议整体是通过: 一次广播 + 一次单播 实现"""
	...
	1.电脑a发现目标主机没有mac,先发送arp广播包,把mac标记成全F的广播地址
	2.交换机接受到arp的广播包,进行从下到上拆包,拆2层,拆到数据链路层看到全F广播地址,
	  开始广播,	把这个广播包发送给每一台主机
	3.每台主机得到广播包后,都开始拆包,如果该数据包找寻的主机ip不是自己,自动舍弃
	4.路由器得到arp广播包后,从下到上拆包,拆3层,拆到网络层,得到网段信息
	5.通过路由器的对照信息表(路由表),找到网段对应的网关(接口)
	6.对应网关的这台交换机得到arp广播包后,从下到上拆包,拆2层,发现全F广播地址进行广播
	7.数据库主机收到广播包后,依次从下到上拆包,发现自己是目标要找的那台主机ip和端口,
	8.把自己的ip->mac对照信息封装,变成arp响应包,发送给对应的交换机
	9.交换机得到arp响应包之后,依次进行单播,返回给最终的原主机
		响应主机--交换机2--路由器--交换机1--请求主机
	10.在回来的过程中,所有得到过相应arp广播包的主机都会自动更新自己的arp解析表,方便下次使用
