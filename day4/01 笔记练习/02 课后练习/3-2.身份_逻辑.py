...
container1 = [1,2,3]
container2 = [1,2,3]
print(container1 is not container2)  #True  py3.7
...
# 逻辑短路
'''
无论后面的表达式是True 还是False 都已经无法改变最后的结果,
那么直接短路,后面的代码不执行;
...
计算规律:
先脑补计算当前的表达式的布尔值是True还是False
如果出现了 True or 表达式 或者 False and 表达式的情况,直接返回前者,后面的代码不执行
如果没有出现短路效果,直接返回后者
'''
...
res = True or False and True or  True
res = True or False or  True
res = True or  True  #返回后者
res =   True
